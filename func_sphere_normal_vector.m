...
function [normal,Err] = func_sphere_normal_vector(handle_in, XYZ, beat_n, step, win_size, start_point, end_point, text, n_vector)
...
for i = 2:count-1 
    ...
    AB=[point_A;point_B];
    plot3(handle_in,AB(:,1),AB(:,2),AB(:,3),'r.','markersize',15)
    ...
    if strcmp(text,'on')
        point_A=1.1*point_A;
        text(handle_in,point_A(1),point_A(2),point_A(3),num2str(i-1));
    end
    point_A = point_B;
end

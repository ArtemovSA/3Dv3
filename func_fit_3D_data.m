function [Err,N,P] = func_fit_3D_data(handle_in, XData, YData, ZData, geometry, visualization, sod)
%
...
switch lower(geometry)
     ...
     case {'on'}
        ...
        cla(handle_in);
        hold(handle_in,'on');
        h = mesh(handle_in, xgrid,ygrid,zgrid,'EdgeColor','black','FaceAlpha',0);
        ...
        switch lower(sod)
            case {'off'}
                plot3(handle_in,X(above,1),X(above,2),X(above,3),'cyan*'); 
            case {'on'}    
                plot3(handle_in,X1',X2',X3','cyan-', X(above,1),X(above,2),X(above,3),'cyano');
        end   
        nbelow = sum(below);
        ...
        switch lower(sod)
            case {'off'}
                plot3(handle_in,X(below,1),X(below,2),X(below,3),'blue*');
            case {'on'}    
                plot3(handle_in,X1',X2',X3','blue-', X(below,1),X(below,2),X(below,3),'blueo');
        end
        N=horzcat([0; 0; 0], N.*500);
        N = N';
        plot3(handle_in,N(:,1),N(:,2),N(:,3),'Linewidth',3);
     case{'off'}
...
%
     ...
     case {'on'}
        cla(handle_in);
        hold(handle_in,'on');
        plot3(handle_in, endpts(:,1),endpts(:,2),endpts(:,3),'black-');
        X1 = [X(:,1) Xfit1(:,1) nan*ones(n,1)];
        X2 = [X(:,2) Xfit1(:,2) nan*ones(n,1)];
        X3 = [X(:,3) Xfit1(:,3) nan*ones(n,1)];
        switch lower(sod)
            case {'off'}
                plot3(handle_in, X(:,1),X(:,2),X(:,3),'blue*');
            case {'on'}    
                plot3(handle_in, X1',X2',X3','cyan-', X(:,1),X(:,2),X(:,3),'blueo');
        end
        %grid;
     case{'off'}
        disp('No visualization.')
